...
def encoder_loss(G, E, D, E_opt, training_set, minibatch_size, reals, beta, labels=None, latent_broadcast=18):
    ...
    fakes = G.components.synthesis.get_output_for(tf.tile(latents[:, np.newaxis], [1, latent_broadcast, 1]), labels)
...
def wp_loss(D, reals, fakes, labels,
    ...
    real_scores_out = fp32(D.components.basic.run(reals, labels))
    fake_scores_out = fp32(D.components.basic.run(fakes, labels))
